...
export default function WorldNews() {
    ...
    const worldNewsQuery = query(collection(db,'posts'), where("category", '==','World News'), orderBy('createdOn','desc'))
 useEffect(()=>{
    const getEntertainment =()=>{
        onSnapshot(worldNewsQuery ,(snapshot)=>{
            setWorldNews(snapshot.docs.map((item)=>({...item.data(), id:item.id})))
        })
  ...
  return (
    <div className='w-full'>
      ...
      <div className='my-[50px] px-4'>
        ...
        {worldNews && worldNews.map(({id,title,content,imageUrl,createdOn,author,category,video}) =>  (
                      <div key={id}> 
                       <Article
